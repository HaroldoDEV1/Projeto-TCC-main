...
import { SafeAreaView, Text, View, TextInput, TouchableOpacity, FlatList, ActivityIndicator, Alert } from 'react-native'
...
export default function DictionaryScreen({ navigation }) {
  ...
  const buscarDefinicao = async () => {
    ...
    try {
      const response = await axios.get(`https://api.wordnik.com/v4/word.json/${termo}/definitions`, {
        params: {
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={styles.definitionContainer}>
              <Text style={styles.term}>{item.word}</Text>
