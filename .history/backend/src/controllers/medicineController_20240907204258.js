...
function ensureDirectoryExistence(filePath) {
    ...
    if (!fs.existsSync(dirname)) {
        fs.mkdirSync(dirname, { recursive: true }) // Cria o diretório se ele não existir
    }
}
...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        ...
        ensureDirectoryExistence(uploadPath) // Garante que o diretório de uploads exista
        cb(null, uploadPath)
    },
...
exports.createMedicine = async (req, res) => {
    try {
        ...
        // Caminho completo para os arquivos de upload
...
exports.updateMedicine = async (req, res) => {
    try {
        ...
        const medicine = await Medicine.findByIdAndUpdate(
            ...
            { new: true, runValidators: true } // Retorna o remédio atualizado e validado
        )
